...
const SubcategoriesAdmin = ()=>{
    ...
    const fetchSubcategories = async ()=>{
        setLoading(true);
        const { data, error } = await supabase.from("insurance_subcategories").select("*, category:insurance_categories(id,name)").eq("is_active", true).order("created_at", {
            ascending: false
        });
        if (error) {
...
//# sourceMappingURL=mokshainvestment_app_admin_subcategories_page_tsx_7229e87c._.js.map